...
#: The clipboard chord is the one keystroke that is not the same everywhere.
#: A Linux *terminal* needs ctrl+shift+c because plain ctrl+c is SIGINT there;
#: Windows uses ctrl+c, and macOS uses command. The clipboard itself stays
#: owned by the focused application either way, so a copied image is still an
#: image -- Sleipnir never reads or rewrites the payload.
_CLIPBOARD_CHORDS = {
    "windows": {"copy": ("ctrl", "c"), "paste": ("ctrl", "v")},
    "macos": {"copy": ("super", "c"), "paste": ("super", "v")},
    "linux": {"copy": ("ctrl", "shift", "c"), "paste": ("ctrl", "shift", "v")},
}


def _clipboard_chord(action: str) -> tuple[str, ...]:
    if platform.IS_WINDOWS:
        ...
        family = "linux"
    return _CLIPBOARD_CHORDS[family][action]


def copy() -> None:
    """Send the platform's copy chord to the focused application."""
    combo = _clipboard_chord("copy")
    key(*combo)
    audit.record("desktop.clipboard_copy", {"combo": "+".join(combo)})


def paste() -> None:
    """Send the platform's paste chord to the focused application."""
    combo = _clipboard_chord("paste")
    key(*combo)
    audit.record("desktop.clipboard_paste", {"combo": "+".join(combo)})

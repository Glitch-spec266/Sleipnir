...
Sleipnir runs on Linux, Windows and macOS from one source tree. Everything the
three operating systems disagree about lives in `src/sleipnir/platform/`, which
binds one name to one backend at import time. macOS shares the POSIX backend
for process control, locking and console mode — that file was checked for
Linux-only syscalls and has none — and gets its own backend only where the
mechanism genuinely differs, which is desktop input and capture. **No module outside that package may
...
| Contract | Linux | Windows | macOS |
|---|---|---|---|
| `try_lock_exclusive` — one process owns a run dir | `fcntl.flock(LOCK_EX\|LOCK_NB)`, advisory | `msvcrt.locking`, **mandatory** | `fcntl.flock`, advisory |
| `create_guarded_launch` — a hard-killed Sleipnir leaves no spending orphan | `prctl(PR_SET_PDEATHSIG)` in `process_guard.py` | nested job object with `KILL_ON_JOB_CLOSE` | **none** — see honest gaps |
| `force_kill_tree` — the whole descendant tree is gone on return | `killpg(SIGKILL)` | `taskkill /F /T` | `killpg(SIGKILL)` |
| `request_group_stop` — ask politely first | `SIGTERM` to the group | `CTRL_BREAK_EVENT` | `SIGTERM` to the group |
| `shell_argv` — one dialect for `CommandCheck` and the operator shell | `/bin/sh -c` | `sh -c` if one exists, else `cmd.exe /c` | `/bin/sh -c` |
| `is_reparse_point` — a path that redirects outside the workspace | symlink | symlink **or NTFS junction** | symlink |
| `key_reader` — raw keystrokes into the console loop | `termios` + `loop.add_reader` | `msvcrt.kbhit` on a polling thread | `termios` + `loop.add_reader` |
| desktop injection | `ydotool` writing `/dev/uinput` | `user32.SendInput` | Quartz `CGEventPost` |
| screen capture | `spectacle`/`grim`/`gnome-screenshot`/`import` | GDI `BitBlt` + a stdlib PNG encoder | `screencapture` |
...
**macOS gates injection by consent, not by isolation.** The App Sandbox does
block `CGEventPost`, but it applies to Mac App Store apps and to nothing else,
and a `pip`-installed CLI is not sandboxed. What stands in the way is TCC:
without the **Accessibility** grant every posted event is *silently dropped* —
the call succeeds and the keystroke never lands. That is the exact failure this
package exists to prevent, so `_darwin.ensure_daemon` refuses to proceed
without the grant rather than letting a caller believe it typed. Two details
cost real debugging time and are therefore in `doctor` rather than in a wiki:
the grant attaches to the *responsible process*, so it is Terminal or iTerm
that appears in System Settings and never Sleipnir itself; and **Screen
Recording** is a separate grant that fails quietly — a capture without it still
succeeds and still writes a PNG, but other applications' windows are blank in
it.
...
macOS has **no parent-death guard**. `WRAPS_CHILDREN` is false there, so a
hard-killed orchestrator can leave worker subprocesses running — and spending.
Linux has `PR_SET_PDEATHSIG` and Windows has job objects; the closest macOS
equivalent is a `kqueue` watchdog on the parent pid, which is a real design
rather than a flag and has not been built. This predates the macOS backend and
is unchanged by it, but it is the gap to close next.

The macOS input path is **written and unit-tested but has never been run
against a real window server.** The backend loads Quartz lazily, so its key
vocabulary, chord flags, consent gate and capture argv are all exercised in CI
